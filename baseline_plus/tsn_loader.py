...
class SimpleVideoDataset:
    def __init__(self, data_file, transform, target_transform=identity, random_select=False, num_segments=None):
        ...
        # ADAPTING TO VIDEO (NOT FRAMES) FILE STRUCTURE
        self.current_video_path = None
        self.current_video_frames = []

    def _load_image(self, directory, idx):
        if self.current_video_path != directory:
            self.current_video_path = directory
            self.current_video_frames = extract_frames(directory)
        try:
            return Image.fromarray(self.current_video_frames[idx - 1]).convert('RGB')
        except Exception:
            print('error loading image:', directory, idx)
            return Image.fromarray(self.current_video_frames[0]).convert('RGB')

    def __getitem__(self,i):
        image_path = self.video_list[i][0]
        ...
        for k in range(self.num_segments):
            img = self._load_image(image_path, frame_id[k])
